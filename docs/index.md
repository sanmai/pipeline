...
-   **Array Conversion**: Use `toList()` to get an array with all values (keys ignored) or `toAssoc()` to get an associative array (keys preserved). The `toArray()` method now requires a callback parameter; `toArrayPreservingKeys()` has been removed.
